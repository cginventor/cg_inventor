...
from . import DEFAULT_VERSION, isValidVersion, normalizeVersion
...
class TagHeader(object):
    ...
    def version(self, v):
        ...
        if not isValidVersion(version, fully_qualified=True):
    ...
    def parse(self, f):
        ...
        minor = ord(version[0])
        rev = ord(version[1])
        log.debug("TagHeader [major]: {}".format(major))
        log.debug("TagHeader [minor]: {}".format(minor))
        log.debug("TagHeader [rev]: {}".format(rev))
        if not (major == 2 and (minor >= 2 and minor <= 4)):
            raise TagException("ID3 v{}.{} is not supported".format(major, minor))
        ...
        args = (bool(b) for b in bytes2bin(data)[0:4])
        self.unsync, self.extended, self.experimental, self.footer = args
        log.debug("TagHeader [flags]: unsync({}) extended({}) "
                  "experimental({}) footer({})" .format(self.unsync, 
                                                        self.extended,
                                                        self.experimental,
                                                        self.footer))
        ...
        log.debug("TagHeader [size]: {} (0x{X})" % (self.tag_size, self.tag_size))
