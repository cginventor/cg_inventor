...
from ..utils import chunkCopy
from ..core  import TXXX_ALBUM_TYPE, TXXX_ARTIST_ORIGIN, ALBUM_TYPE_IDS
from ..id3   import (ANY_VERSION, V1, V1_0, V1_1, V2, V2_2, V2_3, V2_4, versionToString, DEFAULT_LANG, Genre, frames)

from cg_inventor.sys.lib.audio.id3.headers import TagHeader, ExtendedTagHeader
...
class Tag(core.Tag):
    ...
    def parse(self, fileobj, version=ANY_VERSION):
        assert(fileobj)
        self.clear()
        version = version or ANY_VERSION
    ...
    def _loadV1Tag(self, fp):
        ...
        if comment:
            ...
            if comment:
                ...
                self.comments.set(str(comment, v1_enc), V1_COMMENT_DESC)
    ...
    def release_date(self):
        return self._getDate("TDRL") if self.version == V2_4 \
                                     else self._getV23OrignalReleaseDate()


    @release_date.setter
    def release_date(self, date):
        self._setDate("TDRL" if self.version == V2_4 else "TORY", date)
    ...
    def _saveV1Tag(self, version):
        ...
        for c in self.comments:
            if c.description == V1_COMMENT_DESC:
                cmt = c.text
                break
            elif c.description == "":
                cmt = c.text
    ...
    def _saveV2Tag(self, version, encoding, max_padding):
        ...
        if file_exists:
            tmp_tag = Tag()
            if tmp_tag.parse(self.file_info.name, V2):
    ...
    def _convertFrames(self, std_frames, convert_list, version):
        ...
        prefix = "X" if version == V2_4 else "T"
        ...
        if version == V2_3 and "TSST" in [f.id for f in flist]:
    ...
    @staticmethod
    def remove(filename, version=ANY_VERSION, preserve_file_time=False):
        retval = False

        if version[0] & V1[0]:
            tag = Tag()
            with open(filename, "r+b") as tag_file:
                found = tag.parse(tag_file, V1)
                if found:
                    tag_file.seek(-128, 2)
