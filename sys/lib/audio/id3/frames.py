...
import cg_inventor.sys.utils.binary as binary
import cg_inventor.sys.utils.compat as compat

from .. import core
from ..utils import requireUnicode
from .. import Error
from .. import id3
from . import (LATIN1_ENCODING,   UTF_8_ENCODING, 
               UTF_16BE_ENCODING, UTF_16_ENCODING, 
               DEFAULT_LANG)

from .headers import FrameHeader

from cg_inventor.sys.utils.logger import getLogger
...
URL_FIDS           = [URL_COMMERCIAL_FID, URL_COPYRIGHT_FID,
                      URL_AUDIOFILE_FID,  URL_ARTIST_FID, 
                      URL_AUDIOSRC_FID,   URL_INET_RADIO_FID, 
                      URL_PAYMENT_FID,    URL_PUBLISHER_FID]

TOC_FID            = "CTOC"
CHAPTER_FID        = "CHAP"

DEPRECATED_DATE_FIDS = ["TDAT", "TYER", "TIME", "TORY", "TRDA", "XDOR"]
...
class TextFrame(Frame):
    ...
    def render(self):
        ...
        assert(type(self.data) == compat.BytesType)
...
class DateFrame(TextFrame):
    def __init__(self, id, date=""):
        ...
        super(DateFrame, self).__init__(id, text=compat.unicode(date))
        self.date = self.text
        self.encoding = LATIN1_ENCODING
    ...
    def date(self, date):
        ...
        try:
            ...
            elif type(date) is compat.unicode:
        ...
        self.text = compat.unicode(str(date))
...
class UrlFrame(Frame):
    ...
    @url.setter
    def url(self, url):
        if isinstance(url, compat.unicode):
    ...
    def parse(self, data, frame_header):
        ...
        try:
            self.url = compat.unicode(self.data, "ascii").encode("ascii")
...
class UserUrlFrame(UrlFrame):
    ...
    def parse(self, data, frame_header):
        ...
        try:
            self.url = compat.unicode(u, "ascii").encode("ascii")
        except UnicodeDecodeError:
            log.warning("Non ascii url, clearing.")
            self.url = ""
...
class ImageFrame(Frame):
    ...
    def mime_type(self, m):
        m = m or b''
        self._mime_type = m if isinstance(m, compat.BytesType) else m.encode('ascii')
    ...
    def parse(self, data, frame_header):
        ...
        input = compat.StringIO(self.data)
...
class ChapterFrame(Frame):
    ...
    def parse(self, data, frame_header):
        ...
        if data:
            ...
            padding = self.sub_frames.parse(compat.StringIO(data), dummy_tag_header,
                                            ExtendedTagHeader())
        else:
...
class FrameSet(dict):
    ...
    def parse(self, f, tag_header, extended_header):
        ...
        # frame basis.
        if tag_header.unsync and tag_header.version <= id3.V2_3:
        ...
        tag_buffer = compat.StringIO(prepadding + tag_data)
...
def decodeUnicode(bites, encoding):
    ...
    return compat.unicode(bites, codec).rstrip(b"\x00")
...
ID3_FRAMES = { "AENC": ("Audio encryption",
               ...
               "COMM": ("Comments", id3.V2, CommentFrame),
               "COMR": ("Commercial frame", id3.V2, None),

               "CTOC": ("Table of contents", id3.V2, TocFrame),
               "CHAP": ("Chapter", id3.V2, ChapterFrame),

               "ENCR": ("Encryption method registration", id3.V2, None),
               "EQUA": ("Equalisation", id3.V2_3, None),
               "EQU2": ("Equalisation (2)", id3.V2_4, None),
               "ETCO": ("Event timing codes", id3.V2, None),

               "GEOB": ("General encapsulated object", id3.V2, ObjectFrame),
               "GRID": ("Group identification registration", id3.V2, None),

               "IPLS": ("Involved people list", id3.V2_3, None),

               "LINK": ("Linked information", id3.V2, None),

               "MCDI": ("Music CD identifier", id3.V2, MusicCDIdFrame),
               "MLLT": ("MPEG location lookup table", id3.V2, None),

               "OWNE": ("Ownership frame", id3.V2, None),

               "PRIV": ("Private frame", id3.V2, PrivateFrame),
               "PCNT": ("Play counter", id3.V2, PlayCountFrame),
               "POPM": ("Popularimeter", id3.V2, PopularityFrame),
               "POSS": ("Position synchronisation frame", id3.V2, None),

               "RBUF": ("Recommended buffer size", id3.V2, None),
               "RVAD": ("Relative volume adjustment", id3.V2_3, None),
               "RVA2": ("Relative volume adjustment (2)", id3.V2_4, None),
               "RVRB": ("Reverb", id3.V2, None),

               "SEEK": ("Seek frame", id3.V2_4, None),
               "SIGN": ("Signature frame", id3.V2_4, None),
               "SYLT": ("Synchronised lyric/text", id3.V2, None),
               "SYTC": ("Synchronised tempo codes", id3.V2, None),

               "TALB": ("Album/Movie/Show title", id3.V2, TextFrame),
               "TBPM": ("BPM (beats per minute)", id3.V2, TextFrame),
               "TCOM": ("Composer", id3.V2, TextFrame),
               "TCON": ("Content type", id3.V2, TextFrame),
               "TCOP": ("Copyright message", id3.V2, TextFrame),
               "TDAT": ("Date", id3.V2_3, TextFrame),
               "TDEN": ("Encoding time", id3.V2_4, DateFrame),
               "TDLY": ("Playlist delay", id3.V2, TextFrame),
               "TDOR": ("Original release time", id3.V2_4, DateFrame),
               "TDRC": ("Recording time", id3.V2_4, DateFrame),
               "TDRL": ("Release time", id3.V2_4, DateFrame),
               "TDTG": ("Tagging time", id3.V2_4, DateFrame),
               "TENC": ("Encoded by", id3.V2, TextFrame),
               "TEXT": ("Lyricist/Text writer", id3.V2, TextFrame),
               "TFLT": ("File type", id3.V2, TextFrame),
               "TIME": ("Time", id3.V2_3, TextFrame),
               "TIPL": ("Involved people list", id3.V2_4, TextFrame),
               "TIT1": ("Content group description", id3.V2, TextFrame),
               "TIT2": ("Title/songname/content description", id3.V2,
                        TextFrame),
               "TIT3": ("Subtitle/Description refinement", id3.V2, TextFrame),
               "TKEY": ("Initial key", id3.V2, TextFrame),
               "TLAN": ("Language(s)", id3.V2, TextFrame),
               "TLEN": ("Length", id3.V2, TextFrame),
               "TMCL": ("Musician credits list", id3.V2_4, TextFrame),
               "TMED": ("Media type", id3.V2, TextFrame),
               "TMOO": ("Mood", id3.V2_4, TextFrame),
               "TOAL": ("Original album/movie/show title", id3.V2, TextFrame),
               "TOFN": ("Original filename", id3.V2, TextFrame),
               "TOLY": ("Original lyricist(s)/text writer(s)", id3.V2,
                        TextFrame),
               "TOPE": ("Original artist(s)/performer(s)", id3.V2, TextFrame),
               "TORY": ("Original release year", id3.V2_3, TextFrame),
               "TOWN": ("File owner/licensee", id3.V2, TextFrame),
               "TPE1": ("Lead performer(s)/Soloist(s)", id3.V2, TextFrame),
               "TPE2": ("Band/orchestra/accompaniment", id3.V2, TextFrame),
               "TPE3": ("Conductor/performer refinement", id3.V2, TextFrame),
               "TPE4": ("Interpreted, remixed, or otherwise modified by",
                        id3.V2, TextFrame),
               "TPOS": ("Part of a set", id3.V2, TextFrame),
               "TPRO": ("Produced notice", id3.V2_4, TextFrame),
               "TPUB": ("Publisher", id3.V2, TextFrame),
               "TRCK": ("Track number/Position in set", id3.V2, TextFrame),
               "TRDA": ("Recording dates", id3.V2_3, TextFrame),
               "TRSN": ("Internet radio station name", id3.V2, TextFrame),
               "TRSO": ("Internet radio station owner", id3.V2, TextFrame),
               "TSOA": ("Album sort order", id3.V2_4, TextFrame),
               "TSOP": ("Performer sort order", id3.V2_4, TextFrame),
               "TSOT": ("Title sort order", id3.V2_4, TextFrame),
               "TSIZ": ("Size", id3.V2_3, TextFrame),
               "TSRC": ("ISRC (international standard recording code)", id3.V2,
                        TextFrame),
               "TSSE": ("Software/Hardware and settings used for encoding",
                        id3.V2, TextFrame),
               "TSST": ("Set subtitle", id3.V2_4, TextFrame),
               "TYER": ("Year", id3.V2_3, TextFrame),
               "TXXX": ("User defined text information frame", id3.V2,
                        UserTextFrame),

               "UFID": ("Unique file identifier", id3.V2, UniqueFileIDFrame),
               "USER": ("Terms of use", id3.V2, TermsOfUseFrame),
               "USLT": ("Unsynchronised lyric/text transcription", id3.V2,
                        LyricsFrame),

               "WCOM": ("Commercial information", id3.V2, UrlFrame),
               "WCOP": ("Copyright/Legal information", id3.V2, UrlFrame),
               "WOAF": ("Official audio file webpage", id3.V2, UrlFrame),
               "WOAR": ("Official artist/performer webpage", id3.V2, UrlFrame),
               "WOAS": ("Official audio source webpage", id3.V2, UrlFrame),
               "WORS": ("Official Internet radio station homepage", id3.V2,
                        UrlFrame),
               "WPAY": ("Payment", id3.V2, UrlFrame),
               "WPUB": ("Publishers official webpage", id3.V2, UrlFrame),
               "WXXX": ("User defined URL link frame", id3.V2, UserUrlFrame),
}
...
NONSTANDARD_ID3_FRAMES = {
    "NCON": ("Undefined MusicMatch extension", id3.V2, Frame),
    "TCMP": ("iTunes complilation flag extension", id3.V2, TextFrame),
