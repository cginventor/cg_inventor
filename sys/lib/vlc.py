...
def find_lib():
    ...
    if sys.platform.startswith('linux'):
        ...
        library = find_library('libvlc.dll')
        if library is None:
            try:
                if PYTHON3:
                    import winreg
                ...
                for reg in winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER:
                    try:
                        reg = winreg.OpenKey(reg, 'Software\\VideoLAN\\VLC')
                        plugin_path, _ = winreg.QueryValueEx(reg, 'InstallDir')
                        winreg.CloseKey(reg)
                        break
                    except winreg.error:
                        pass                    
            ...
            if plugin_path is None:
                for path in ('Program Files\\VideoLan\\', 'VideoLan\\', 'Program Files\\', ''):
                    path = 'C:\\%sVLC\\libvlc.dll' %path
